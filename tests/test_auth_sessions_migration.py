"""Migration 0052 must create exactly the tables the models declare
(create-only; tests/test_compliance_rules_migration.py technique)."""
...
MIGRATION = "0052_auth_sessions.py"
TABLES = ["auth_sessions", "security_audit_events"]
_PREREQ = ["users"]
...
def test_the_revision_follows_0051():
    module = _load(MIGRATION)
    assert module.revision == "0052_auth_sessions"
    assert module.down_revision == "0051_compliance_rules"


@pytest.mark.parametrize("table", TABLES)
def test_table_matches_the_model(from_models, from_migration, table):
...
def test_downgrade_round_trips(tmp_path):
    ...
    assert not remaining.intersection(TABLES)
    engine.dispose()
