"""Migration 0031 (Feature Group 9) must produce exactly the schema the models
declare. It only creates tables, so the prerequisites come straight from the
metadata copy (no columns to strip)."""
...
NEW_TABLES = ["deliverability_checks", "compliance_audit_log"]
...
def from_migration(tmp_path):
    ...
    with engine.begin() as connection:
        with Operations.context(MigrationContext.configure(connection)):
            _load("0031_trust_deliverability.py").upgrade()
    yield sa.inspect(engine)
    engine.dispose()


@pytest.mark.parametrize("table", NEW_TABLES)
def test_tables_match(from_models, from_migration, table):
...
def test_downgrade_round_trips(tmp_path):
    ...
    _build_prereqs(engine)
    module = _load("0031_trust_deliverability.py")
    with engine.begin() as connection:
        with Operations.context(MigrationContext.configure(connection)):
            module.upgrade()
            module.downgrade()
