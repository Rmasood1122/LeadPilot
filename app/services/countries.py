...
The single source of truth for "is this a country": onboarding validates
against it and GET /onboarding/countries serves it to the frontend, so the
select the user picks from and the check the server applies cannot drift.
Codes are stored upper case, exactly as IP-geolocation providers return them.
"""
...
COUNTRIES: dict[str, str] = {
    ...
    "CG": "Congo (Republic)", "CH": "Switzerland", "CI": "Côte d'Ivoire",
...
def normalize_country(value: str | None) -> str | None:
    """Upper-cased code when it is a known country, else None."""
    code = (value or "").strip().upper()
    return code if code in COUNTRIES else None


def country_list() -> list[dict]:
    """[{code, name}] sorted by name -- the order a person scans a select in."""
